...
The `tournaments` module exports **five** routers from `routers/__init__.py` (`router`, `bout_router`, `engine_router`, `intake_router`, `read_router`), all mounted side by side in `app/main.py`. Do not nest them: `bout`/`engine`/`intake`/`read` carry their own `/api/v1` prefix, so including any of them inside the `/api/v1/tournaments`-prefixed router doubles the prefix. Writes live in `engine.py`/`tournaments.py`/`bouts.py`/`intake.py`; every read projection (competition/participant/team/match views with resolved names, standings, bracket tree, journal) lives in `read.py` + `services/read_service.py` + `schemas/views.py`.
