...
def register_organizer(client, email: str = "organizer@example.com") -> tuple[str, dict[str, str]]:
    """Register a user and return their id plus a snapshot of their session.
...
def bootstrap(client, entrants: list[tuple[str, str]], *, competition_format="SINGLE_ELIMINATION"):
    ...
    organizer_id, session = register_organizer(client)
    ...
    return tournament_id, competition_id, participant_ids, session


def generate(client, competition_id: str, session: dict[str, str], **body):
    use_session(client, session)
    return client.post(
        f"/api/v1/competitions/{competition_id}/bracket/generate", json=body
    )
...
def test_four_participants_four_cities_gives_a_clean_bracket():
    ...
    response = generate(client, competition_id, session)
    assert response.status_code == 201, response.text
    plan = response.json()
...
def test_eight_participants_avoid_same_city_first_round_pairs():
    ...
    _, competition_id, _, session = bootstrap(client, entrants)
...
def test_preview_does_not_write_anything():
    ...
    preview = client.post(
        f"/api/v1/competitions/{competition_id}/bracket/preview"
    )
...
def test_regenerating_an_existing_bracket_is_refused():
    ...
    assert generate(client, competition_id, session).status_code == 201
    assert generate(client, competition_id, session).status_code == 409
...
def test_bracket_generation_requires_an_authorized_manager():
    ...
    register_organizer(client, email="stranger@example.com")
...
def test_an_instructor_may_run_someone_elses_tournament():
    ...
    instructor_id, instructor_session = register_organizer(client, email="instructor@example.com")
    ...
    use_session(client, instructor_session)
    response = client.post(
        f"/api/v1/competitions/{competition_id}/bracket/generate", json={}
    )
    assert response.status_code == 201, response.text
